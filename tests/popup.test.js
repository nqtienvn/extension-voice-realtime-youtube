...
test("voice ngoại ngữ cũ không tự kích hoạt nếu chưa được chọn lại", () => {
  const legacyVoice = {
    name: "English legacy",
    lang: "en-US",
  ...
  assert.equal(harness.voice.value, "");
  assert.equal(harness.persistedSettings.allowAnyVoice, undefined);
});

test("popup liệt kê mọi voice nhưng vẫn ưu tiên giọng Việt ở đầu", () => {
  const harness = createHarness(
    { ttsSettingsVersion: 1 },
    [
      {
  ...
  assert.deepEqual(
    ...
    ["", "voice-vi-vn", "voice-vi", "voice-en"]
  );
  assert.match(harness.voiceHint.textContent, /3 giọng/);
  assert.match(harness.voiceHint.textContent, /giọng tiếng Việt được ưu tiên/);
});

test("đổi sang voice bất kỳ và cho phép trở lại chế độ tự động", () => {
  const selectedVoice = {
    name: "English",
    lang: "en-US",
    voiceURI: "voice-en",
  ...
  harness.voice.value = selectedVoice.voiceURI;
  ...
  assert.equal(harness.persistedSettings.allowAnyVoice, false);
});
